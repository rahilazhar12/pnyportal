...
const JobPage = () => {
  ...
  if (!jobData) {
    return <div className="p-6 text-center text-red-500">No Job Data Available</div>;
  }
  ...
  const applyForJob = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/v1/users/jobs/apply/${jobData._id}`, // Use jobData._id from state
        {
          method: "POST",
          headers: {
    ...
    <div className="flex flex-col md:flex-row gap-6 p-4 md:px-10 lg:px-24 bg-gray-200">
      ...
      <div className="w-full md:w-2/3 bg-white shadow-lg rounded-lg px-6 py-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
          ...
          <img
            src={`${import.meta.env.VITE_API_URL}/uploads/${jobData.companyLogo}`}
            alt="Company Logo"
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2 space-y-2 sm:space-y-0">
          <span className="text-lg font-semibold text-center sm:text-left">
            PKR {jobData.maxPrice} • {jobData.salaryType} • {jobData.experienceLevel}
          </span>
          <button
            onClick={applyForJob}
          ...
          >
            ...
            <span>{loadingJobId === jobData._id ? "Applying..." : "Apply Now"}</span>
          </button>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {jobData.skillsRequired.map((skill, index) => (
              <span key={index} className="bg-blue-100 text-blue-800 px-4 py-1 rounded-full border border-blue-300">
                {skill.trim()}
              </span>
            ))}
          </div>
        ...
        <div className="mt-6">
          {activeTab === "JobDescription" && (
            <p className="text-gray-700 mb-4 text-sm" dangerouslySetInnerHTML={{ __html: jobData.description }}></p>
          )}
          {activeTab === "AboutCompany" && (
            <div className="text-gray-700 space-y-4 text-sm" dangerouslySetInnerHTML={{ __html: jobData.about || "Company information is not available." }}></div>
          )}
        </div>
      </div>
    </div>
